...
export function estimateOpenAiTextCost({ model, inputTokens = 0, cachedInputTokens = 0, cacheWriteInputTokens = 0, outputTokens = 0 } = {}) {
  const rateKey = resolveRateKey(model);
  const rates = rateKey ? textRates[rateKey] : null;
  if (!rates) return { estimatedCostMicrousd: null, pricingVersion: openAiPricingVersion, rateKey: null };
  ...
  return { estimatedCostMicrousd, pricingVersion: openAiPricingVersion, rateKey };
}
