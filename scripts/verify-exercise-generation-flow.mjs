...
if (!isLoopback(target.hostname) && !process.argv.includes("--allow-remote")) {
  throw new Error("Remote exercise-generation checks require --allow-remote.");
}
...
const email = `stonecode.exercise-generation.${suffix}@example.test`;
...
try {
  const { data: created, error: createError } = await admin.auth.admin.createUser({
    ...
    password,
    email_confirm: true,
    user_metadata: { display_name: "Exercise Generation QA" }
  });
  ...
  const brief = {
    type: "exercise",
    goal: "Practice Python loops for coding interviews",
    subject: "Python loops",
    language: "Python",
    motivation: "Prepare for coding interviews",
    practiceScope: "topics",
    topics: ["for loops", "while loops", "loop control"],
    difficulty: "adaptive",
    exerciseCount: 10,
  ...
  const proposalResult = await apiJson("/api/learning/proposals", token, {
    ...
    body: JSON.stringify({ brief, idempotencyKey: `exercise-proposal:${suffix}` })
  });
  assert.equal(proposalResult.proposal.type, "exercise");
  assert.equal(proposalResult.proposal.totals.exercises, 10);
  assert.equal(proposalResult.proposal.creditQuote.credits, 2);

  const finalized = await apiJson(`/api/learning/proposals/${proposalResult.proposal.id}/finalize`, token, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ idempotencyKey: `exercise-finalize:${suffix}` })
  });
  assert.ok(finalized.job.id);
  ...
  assert.equal(course.experience_type, "exercise");
  const content = normalizeGeneratedLearningContent(course.course_content, { brief });
  assert.equal(content.problems.length, 10);
  assert.equal(content.problems.filter((problem) => problem.kind === "code").length, 7);
  assert.equal(content.problems.filter((problem) => problem.kind === "mcq").length, 3);
  assert.ok(content.problems.every((problem) => problem.blocks.length === 1 && problem.blocks[0].steps.length === 1));

  const credits = await apiJson("/api/credits", token);
  assert.equal(credits.credits.available, 8);
  console.log(JSON.stringify({
    ...
    codingCount: 7,
    mcqCount: 3,
    remainingCredits: credits.credits.available
  }, null, 2));
} finally {
...
async function waitForJob(jobId, token) {
  ...
  throw new Error("Exercise generation did not finish within five minutes.");
}
