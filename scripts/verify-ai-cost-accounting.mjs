...
import { estimateOpenAiTextCost, formatCostUsd, nominalProStoneUsd, openAiFastPricingVersion, openAiPricingVersion } from "../server/billing/ai-costs.mjs";
...
const fastLunaEstimate = estimateOpenAiTextCost({
  model: "gpt-5.6-luna",
...
assert.equal(fastLunaEstimate.serviceTier, "fast");
assert.equal(fastLunaEstimate.pricingVersion, openAiFastPricingVersion);
assert.equal(fastLunaEstimate.estimatedCostMicrousd, 51_380);
assert.equal(formatCostUsd(fastLunaEstimate.estimatedCostMicrousd), 0.05138);
